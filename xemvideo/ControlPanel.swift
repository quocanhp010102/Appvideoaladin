...
private extension ControlPanel {
    ...
    @objc func didTapRewind() {
        // Xử lý hành động khi nhấn nút tua lại
        // Tương tự như các phương thức trên, bạn cần thông báo cho delegate
        delegate?.controlPanel(self, didTapRewindButton: rewindButton)
    }

    @objc func didTapFastForward() {
        // Xử lý hành động khi nhấn nút tua tới
        // Tương tự như các phương thức trên, bạn cần thông báo cho delegate
        delegate?.controlPanel(self, didTapFastForwardButton: fastForwardButton)
    }
}
